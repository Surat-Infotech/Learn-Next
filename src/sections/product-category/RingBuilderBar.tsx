...
const RingBuilderBar: FC<IRingBuilderBarProps> = (props) => {
  const { currentStep, currentShape } = props;

  const pathname = usePathname();
  const { query } = useRouter();
  const { shapeMatch } = useInventoryContext();
  ...
  const isShowChooseRing = useMemo(() => {
    ...
    if (pathname.includes('/engagement-rings/') && currentShape && query.c_type) {
    ...
    return false;
  }, [_selected_shape, currentShape, pathname, query.c_type, query.shape, shapeMatch]);

  const ringSettingUrl = useMemo(
    () =>
      ringSetting?.product?._id
        ? paths.product.details(`engagement-rings/${ringSetting.product?.display_slug}?view=true`)
        : '',
    [ringSetting]
  );
  ...
  const ringDiamondUrl = useMemo(() => {
    ...
    return `${url}?type=ring-builder&shape=${ringDiamond.diamond.shape}`;
  }, [ringDiamond]);

  const ringSizeUrl = useMemo(
    () => (ringSetting?.product?._id ? paths.ringPreview.details(ringSetting.product._id) : ''),
    [ringSetting]
  );
  ...
  return (
    <div className="container-fluid px-0 px-lg-3">
      <div className="ringbuilder_brdcrmb">
        ...
        >
          <span className="breadcrumb__inner">
            <span className="breadcrumb__number ms-0">1</span>
              ...
              <div className="breadcrumb__details">
                ...
                <div>
                  ...
                  <div className="d-flex">
                    <button
                      type="button"
                      onClick={() => [resetRingSetting(), setchooseSettingActive(true)]}
                    >
                      Clear
                    </button>
        ...
        <span
          className={clsx(
            (chooseDiamondActive === true || !isShowChooseRing
              ? ''
              : currentStep === 'choose-diamond' ||
                currentStep === 'choose-size' ||
        ...
        >
          <span className="breadcrumb__inner">
            <span className="breadcrumb__number">2</span>
              ...
              <div className="breadcrumb__details">
                ...
                <div>
                  ...
                  <div className="d-flex">
                    ...
                    <Link
                      href={`${paths.diamondDetail.details(ringDiamond.diamond_type as 'white' | 'color', ringDiamond.diamond._id as string)}?type=ring-builder`}
                    >
                      View
                    </Link>
                  </div>
            ...
            {ringDiamond && ringSetting && isShowChooseRing ? (
              <div className="breadcrumb__details">
                ...
                <div>
                  ...
                  <div className="d-flex">
                    <button type="button" onClick={resetRingBuilder}>
                      <Link className="m-0 p-0" href={paths.buildRing.root}>
                        Clear
                      </Link>
                    </button>
