...
const LoginForm: FC<ILoginFormProps> = (props) => {
  ...
  const onSubmit = async (formData: typeof defaultValues) => {
    ...
    try {
      ...
      if (e.response.data.status === 401) signOut({ callbackUrl: paths.order.root }); localStorage.clear();
    ...
    <>
      <form onSubmit={handleSubmit(onSubmit)} className={clsx(className)} {...other}>
        ...
        <div className="d-flex align-items-center">
          <button type="submit" className="common_btn me-3" disabled={isSubmitting}>
            {isSubmitting ? (
