...
export type ColorInventoryTableRawProps = IColorDiamond & {
  onAddToBag: () => void;
...
const ColorInventoryTableRaw: FC<ColorInventoryTableRawProps> = (props) => {
  ...
  const diamond_color = colorDaimondFilters.find((_c) => _c.defaultValue === Number(color))?.color;
  const clarity = clarityFilters.find((_c) => _c.value === Number(_clarity))?.label_view;
  const intensity = intensityFilters.find((_c) => _c.defaultValue === Number(_intensity))?.label;
  ...
  const intensityDescription = intensityFilters.find(
    (_c) => _c.defaultValue === Number(_intensity)
  )?.description;
  ...
  const {
    ...
    ...colorinventoriesQuery.get(inventoriesQueryGetId),
    enabled: false,
  });
  ...
  const handleViewMore = useCallback(
    (id?: string) => {
      ...
      if (
        ...
        if (query.type) {
          router.push(`${paths.diamondDetail.details('color', _id as string)}?type=${query.type}`);
        } else {
          router.push(
            `${paths.diamondDetail.details('color', _id as string)}?c_type=${query.c_type}`
          );
        }
      } else {
  ...
  const CheckDiamondexistOrNot = diamondSku?.find((item) => item.sku === sku)?.sku;
            ...
            <td colSpan={7} className="view-product position-relative">
              ...
              <div className="container-fluid">
                <div className="row gy-3 gy-lg-5">
                  ...
                  <div className="col-md-6">
                    <div className="ls_wishlist_dmnd text-center">
                      <div className="d-flex justify-content-center align-items-center position-relative">
                        {/* Image */}
                        <Image
                          src={`/assets/images/colorDiamond/${_shape}/${diamond_color}.png`}
                          alt="colorDiamond"
                          width={180}
                          height={180}
                        />
                      ...
                      {/* CTA */}
                      <div className="d-flex align-items-center justify-content-center gap_5">
                        {/* Add to cart btn */}
                        {query.type ? (
                          <>
                            {!addToSettingLoader &&
                      ...
                      <p className="dmnd_msg mb-0 mt-3">
                  ...
                  {/* Right section */}
                  <div className="col-md-6">
                    <p className="fw-600 mb_10">DIAMOND DETAILS</p>
                    {/* Carat */}
                    {carat && (
                      ...
                      <div className="diamond_detail">
                        <div className="row">
                          ...
                          <div className="col-9 px-8">
                            <div className="odesc">
                              <h6>{intensity}</h6>
                              <span>{intensityDescription}</span>
                            </div>
                          </div>
                        </div>
                      </div>
...
export default ColorInventoryTableRaw;
